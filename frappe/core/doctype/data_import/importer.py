        ...
        for batch_index, batched_payloads in enumerate(
            ...
            for i, payload in enumerate(batched_payloads):
                ...
                try:
                    ...
                    eta = self.get_eta(
                        current_index, total_payload_count, processing_time
                    )
...
class ImportFile:
    ...
    def parse_data_from_template(self):
        ...
        for i, row in enumerate(self.raw_data):
            ...
            if not header:
                header = Header(
                    i, row, self.doctype, self.raw_data, self.column_to_field_map
                )
            else:
            ...
            for doctype, table_df in doctypes:
                ...
                if doctype != self.doctype and table_df:
                    ...
                    if child_doc is None:
                        continue
                    parent_doc[table_df.fieldname] = parent_doc.get(
                        table_df.fieldname, []
                    )
...
class Column:
    ...
    def __init__(
        self, index, header, doctype, column_values, map_to_field=None, seen=[]
    ):
        self.index = index
    ...
    def parse(self):
                ...
                self.warnings.append(
                    {
                        "message": _("Mapping column {0} to field {1}").format(
                            frappe.bold(header_title or "<i>Untitled Column</i>"),
        ...
        if header_title in self.seen:
            ...
            self.warnings.append(
                {
                    "col": column_number,
                    "message": _("Skipping column {0}").format(
                        frappe.bold(header_title)
                    ),
                    "type": "info",
                }
            ...
            self.warnings.append(
                {
                    "col": column_number,
                    "message": _("Skipping Untitled Column"),
                    "type": "info",
                }
            )
        ...
        if self.df.fieldtype == "Link":
            ...
            exists = [
                ...
                for d in frappe.db.get_all(
                    self.df.options, filters={"name": ("in", values)}
                )
            ]
...
def build_fields_dict_for_column_matching(parent_doctype):
    ...
    for doctype, table_df in doctypes:
        ...
        name_by_fieldname = (
            ...
            else "{0}.name".format(table_df.fieldname)
        )
            ...
            if fieldtype not in no_value_fields:
                if parent_doctype == doctype:
                    ...
                    table_fields = parent_meta.get(
                        "fields",
                        {"fieldtype": ["in", table_fieldtypes], "options": parent},
                    )
                    for table_field in table_fields:
                        by_label = "{0} ({1})".format(label, table_field.label)
                        by_fieldname = "{0}.{1}".format(
                            table_field.fieldname, df.fieldname
                        )
...
def get_df_for_column_header(doctype, header):
    ...
    df_by_labels_and_fieldname = frappe.cache().hget(
        ...
        doctype,
        generator=build_fields_dict_for_doctype,
    )
