...
class Database(object):
    ...
    def get_values(
        ...
        if isinstance(filters, list):
            out = self._get_value_for_many_names(
                doctype, filters, fieldname, debug=debug
            )
                ...
                try:
                    ...
                    if ignore and (
                        ...
                        out = self.get_values_from_single(
                            fields, filters, doctype, as_dict, debug, update
                        )
    ...
    def _get_values_from_table(
        ...
        r = self.sql(
            "select {fields} from `tab{doctype}` {where} {conditions} {order_by} {for_update}".format(
