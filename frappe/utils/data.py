...
def is_invalid_date_string(date_string):
    ...
    return not isinstance(date_string, str) or (
        (not date_string)
        or (date_string or "").startswith(("0001-01-01", "0000-00-00"))
    )
...
def get_quarter_start(dt, as_str=False):
    ...
    return (
        first_date_of_quarter.strftime(DATE_FORMAT) if as_str else first_date_of_quarter
    )
        ...
        if precision > 2:
            if len(decimals) < 3:
                if currency:
                    fraction = (
                        frappe.db.get_value(
                            "Currency", currency, "fraction_units", cache=True
                        )
                        or 100
                    )
...
def get_thumbnail_base64_for_image(src):
    ...
    def _get_base64():
        ...
        return {
            "base64": "data:image/{0};base64,{1}".format(
                extn, safe_decode(base64_string)
            ),
    ...
    if isinstance(some_list, (list, tuple)):
        ...
        if not some_list:
            ...
            some_list = (
                ...
                if add_quotes
                else ["%s" % s for s in some_list]
            )
            return pattern.format(
                ", ".join(frappe._(s) for s in some_list[:-1]), some_list[-1]
            )
    else:
        return some_list
...
def get_url(uri=None, full_address=False):
    ...
    if not host_name:
        ...
        if request_host_name:
            ...
            host_name = frappe.db.get_value(
                "Website Settings", "Website Settings", "subdomain"
            )
    ...
    if (
        not (
            frappe.conf.restart_supervisor_on_update
...
def sanitize_column(column_name):
    ...
    regex = re.compile("^.*[,'();].*")
    column_name = sqlparse.format(
        column_name, strip_comments=True, keyword_case="lower"
    )
    ...
    if "ifnull" in column_name:
        if regex.match(column_name):
            # to avoid and, or
            if any(
                " {0} ".format(keyword) in column_name.split()
            ...
            # to avoid select, delete, drop, update and case
                ...
                keyword in column_name.split() for keyword in blacklisted_keywords
            ):
                _raise_exception()
...
def expand_relative_urls(html):
    ...
    def _expand_relative_urls(match):
        ...
        if not to_expand[2].startswith("mailto") and not to_expand[2].startswith(
            "data:"
        ):
