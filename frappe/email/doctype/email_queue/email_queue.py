...
class SendMailContext:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type in exceptions:
            ...
            if self.queue_doc.retry < MAX_RETRY_COUNT:
                ...
                update_fields = {
                    "status": "Partially Errored" if self.sent_to else "Error"
                }
            self.queue_doc.update_status(**update_fields, commit=True)
        else:
            email_status = (
                "Sent"
                if self.is_mail_sent_to_all()
                else ("Partially Sent" if self.sent_to else "Not Sent")
            )
    ...
    def log_exception(self, exc_type, exc_val, exc_tb):
        if exc_type:
            ...
            if self.is_background_task:
                frappe.log_error(
                    title="frappe.email.queue.flush", message=traceback_string
                )
            else:
    ...
    def build_message(self, recipient_email):
        ...
        message = message.replace(
            self.message_placeholder("tracker"), self.get_tracker_str()
        )
        message = message.replace(
    ...
    def get_tracker_str(self):
        ...
        if frappe.conf.use_ssl and self.email_account_doc.track_email_status:
            message = quopri.encodestring(
                tracker_url_html.format(
                    frappe.local.site, self.queue_doc.communication
                ).encode()
            ).decode()
        return message
    ...
    def get_unsubscribe_str(self, recipient_email):
        ...
        if self.queue_doc.add_unsubscribe_link and self.queue_doc.reference_doctype:
            doctype, doc_name = (
                self.queue_doc.reference_doctype,
                self.queue_doc.reference_name,
            )
...
def on_doctype_update():
    ...
    frappe.db.add_index(
        "Email Queue",
        ("status", "send_after", "priority", "creation"),
        "index_bulk_flush",
    )
...
class QueueBuilder:
    ...
    def unsubscribe_message(self):
        if self.should_include_unsubscribe_link():
            return get_unsubscribe_message(
                self._unsubscribe_message, self.expose_recipients
            )
    ...
    def final_recipients(self):
        ...
        return [
            mail_id for mail_id in self.recipients if mail_id not in unsubscribed_emails
        ]
    ...
    def process(self, send_now=False):
        ...
        if not queue_separately:
            ...
            for r in final_recipients:
                recipients = (
                    [r] if email_queues else list(set([r] + self.final_cc() + self.bcc))
                )
                q = EmailQueue.new(
                    {**queue_data, **{"recipients": recipients}},
