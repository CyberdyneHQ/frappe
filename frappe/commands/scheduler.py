...
def _is_scheduler_enabled():
    ...
    try:
        ...
        enable_scheduler = (
            True
            if cint(frappe.db.get_single_value("System Settings", "enable_scheduler"))
            else False
        )
    except:
        pass
...
def ready_for_migration(context, site=None):
    ...
    try:
        ...
        if pending_jobs:
            print(
                "NOT READY for migration: site {0} has pending background jobs".format(
                    site
                )
            )
            sys.exit(1)
            ...
            print(
                "READY for migration: site {0} does not have any background jobs".format(
                    site
                )
            )
            return 0
