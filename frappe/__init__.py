    ...
    if site_path:
        ...
        if os.path.exists(site_config):
            try:
                ...
                click.secho(
                    "{0}/site_config.json is invalid".format(local.site), fg="red"
                )
                print(error)
...
def has_permission(
    doctype=None, ptype="read", doc=None, user=None, verbose=False, throw=False
):
    ...
    if throw and not out:
        if doc:
            frappe.throw(
                _("No permission for {0}").format(doc.doctype + " " + doc.name)
            )
        else:
